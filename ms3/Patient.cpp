...
namespace seneca {
   ...
   std::ostream& Patient::write(std::ostream& ostr) const {
    if (&ostr == &std::clog) {
        ostr << std::left << std::setw(53) << std::setfill('.') << m_name << std::right << m_OHIP << "   " << m_ticket;
    }
    else if (&ostr == &std::cout) {
        if (m_name != nullptr) {
            m_ticket.write(ostr) << std::endl; // Ensure this line formats the time correctly
        ...
        m_ticket.write(ostr); // Ensure this line formats the time correctly
    }
    return ostr;
}
